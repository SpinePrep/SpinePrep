...
def _link_cohort_deliverables(view_root: Path, source_wf: Path) -> None:
    ...
    S10 emits its release artifacts to one of two places (audit F11):
      ...
      * ``source_wf/derivatives/spineprep/`` (legacy)
    ...
    # Legacy fallback: cohort-tier files in derivatives/spineprep/
    src_top = source_wf / "derivatives" / "spineprep"
    if not src_top.exists():
        return
    dest_top = view_root / "derivatives" / "spineprep"
...
def build_view(scope: str, work_root: Path) -> tuple[Path, set[str], dict[str, str]]:
    ...
    for n in range(1, MAX_STEP + 1):
        ...
        # via the dashboard's ``../derivatives/spineprep/`` path.
        if n == 11:
