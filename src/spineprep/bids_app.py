"""Standard BIDS-App entrypoint for SpinePrep.

Conformant interface (BIDS-Apps convention, Gorgolewski 2017):

    spineprep <bids_dir> <output_dir> <analysis_level> [options]
...
def _acquisition_envelope_warnings(
    ...
    SpinePrep is validated on cervical-cord EPI-BOLD at 3T. We warn on what
    ...
    for sub in subs:
        ...
        if seq and "EP" not in seq:
            warnings.append(
                ...
                "look like EPI; SpinePrep is validated on EPI-BOLD.")
    return warnings
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="spineprep",
        description="SpinePrep — BIDS-App for spinal-cord fMRI preprocessing.",
    )
...
def run_bids_app(
    ...
    from spineprep import cli  # in-process: no PATH/poetry dependency
    ...
    def _finish(status: str, code: int) -> int:
        ...
        try:
            (output_dir / "spineprep_run_manifest.json").write_text(
    ...
    for step in steps:
        ...
        if step == GROUP_STEP:
            ...
            deriv_problems = validate_derivatives(
                output_dir / "derivatives" / "spineprep")
...
def main_bids_app(argv: Optional[list[str]] = None) -> int:
    ...
    if getattr(args, "version", False):
        from spineprep import __version__ as v  # noqa
        print(v)
        return 0
    if getattr(args, "smoothing_sigma_mm", None):
        import os
        os.environ["SPINEPREP_SIGMA_MM"] = " ".join(
