...
from spineprep.lib.run import run_command as _run_command  # noqa: F401
from spineprep.lib.run import is_command_not_found as _is_command_not_found  # noqa: F401
...
def _derivatives_anat_dir(out_root: Path, subject: str, session: Optional[str], dataset_key: Optional[str] = None) -> Path:
    ...
    out_root/derivatives/spineprep/{dataset_key}/sub-XX/[ses-XX/]anat

    If dataset_key is None, uses the traditional path:
    out_root/derivatives/spineprep/sub-XX/[ses-XX/]anat
    """
    if dataset_key:
        base = out_root / "derivatives" / "spineprep" / dataset_key / f"sub-{subject}"
    else:
        base = out_root / "derivatives" / "spineprep" / f"sub-{subject}"
...
def _derivatives_figures_dir(out_root: Path, subject: Optional[str], session: Optional[str], dataset_key: Optional[str] = None) -> Path:
    ...
    If dataset_key is provided, includes it in the path for multi-dataset workflows:
    out_root/derivatives/spineprep/{dataset_key}/sub-XX/[ses-XX/]figures

    If dataset_key is None, uses the traditional path:
    out_root/derivatives/spineprep/sub-XX/[ses-XX/]figures
    """
    if dataset_key:
        if session:
            return out_root / "derivatives" / "spineprep" / dataset_key / f"sub-{subject}" / f"ses-{session}" / "figures"
        return out_root / "derivatives" / "spineprep" / dataset_key / f"sub-{subject}" / "figures"
    else:
        if session:
