...
def _render_reportlets_for_runs(runs: list[dict], out_root: Path,
    ...
    updated = []
    for run in runs:
        # Render QC reportlets for kept runs (PASS and WARN). WARN runs flow
        # downstream and are exactly the ones a human most needs to eyeball, so
        # they must get figures too; only skip hard FAILs (missing artifacts).
