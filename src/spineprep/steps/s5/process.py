...
def _resolve_ants_binary(tool: str) -> Optional[str]:
    ...
    (``isct_antsRegistration``, as shipped in the SpinePrep container).
...
def _ants_command(cmd: list[str]) -> list[str]:
    ...
    The Docker path mounts the SpinePrep project root so absolute paths
...
def run_S5_func_distortion_correction(
    ...
    # Output paths
    if session:
        func_dir = out_dir / "derivatives" / "spineprep" / f"sub-{subject}" / f"ses-{session}" / "func"
    else:
        func_dir = out_dir / "derivatives" / "spineprep" / f"sub-{subject}" / "func"
