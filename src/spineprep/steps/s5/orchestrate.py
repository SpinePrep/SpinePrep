...
def _anat_search_roots(
    ...
    for base in bases:
        # Layout A: derivatives/spineprep/<dataset_key>/sub-XX/[ses-YY]/anat/
        roots.append(base / "derivatives" / "spineprep" / dataset_key
                     / f"sub-{sub_norm}" / (f"ses-{ses_norm}" if ses_norm else "") / "anat")
        # Layout B: derivatives/spineprep/sub-XX/[ses-YY]/anat/
        roots.append(base / "derivatives" / "spineprep"
                     / f"sub-{sub_norm}" / (f"ses-{ses_norm}" if ses_norm else "") / "anat")
...
def run_S5(
    ...
    for s4_run in s4_runs:
        ...
        if ses:
            mocoref = (out_path / "derivatives" / "spineprep"
                       / f"sub-{subj}"
                       / f"ses-{ses}" / "func"
