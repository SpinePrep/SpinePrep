...
def main(argv: list[str] | None = None) -> int:
    # Standard BIDS-App invocation: `spineprep BIDS_DIR OUTPUT_DIR LEVEL`.
    ...
    if _argv and _argv[0] not in ("run", "check") and not _argv[0].startswith("-"):
        from spineprep.bids_app import main_bids_app
...
def _run_S1(args):
    ...
    if dataset_keys:
        ...
        from spineprep.S1_input_verify import run_S1_input_verify_batch
...
def _run_S2(args):
    ...
    if args.reportlets_only:
        ...
        if dataset_keys:
            ...
            from spineprep.S2_anat_cordref import run_S2_anat_cordref_reportlets_only_batch
...
def _resolve_scope_to_dataset_keys(scope: str) -> list[str]:
    ...
    """
    from spineprep.policy.datasets import load_dataset_policy
...
def _run_S2B(args):
    from spineprep.S2B_func_denoise import run_S2B_func_denoise
...
def _check_S2B(args):
    from spineprep.S2B_func_denoise import check_S2B_func_denoise
...
def _run_S3(args):
    ...
    if args.reportlets_only:
        ...
        if dataset_keys:
            from spineprep.S3_func_init_and_crop import run_S3_func_init_and_crop_reportlets_only_batch
            ...
            from spineprep.S3_func_init_and_crop import run_S3_func_init_and_crop_reportlets_only
    ...
    if dataset_keys:
        # Batch mode: process multiple datasets
        from spineprep.S3_func_init_and_crop import run_S3_func_init_and_crop_batch
...
def _run_S4(args):
    from spineprep.S4_func_motion_correction import run_S4, StepResult
    ...
    if args.reportlets_only:
        if dataset_keys:
            from spineprep.S4_func_motion_correction import run_S4_func_motion_correction_reportlets_only_batch
            ...
            from spineprep.S4_func_motion_correction import run_S4_func_motion_correction_reportlets_only
...
def _check_S4(args):
    from spineprep.S4_func_motion_correction import check_S4_func_motion_correction
...
def _run_S5(args):
    from spineprep.S5_func_distortion_correction import (
        run_S5, StepResult,
...
def _check_S5(args):
    from spineprep.S5_func_distortion_correction import check_S5_func_distortion_correction
...
def _run_S6(args):
    from spineprep.S6_func_to_anat_registration import (
        run_S6, StepResult,
...
def _check_S6(args):
    from spineprep.S6_func_to_anat_registration import check_S6_func_to_anat_registration
...
def _run_S7(args):
    from spineprep.S7_template_normalization import (
        run_S7, StepResult,
...
def _check_S7(args):
    from spineprep.S7_template_normalization import check_S7_template_normalization
...
def _run_S8(args):
    from spineprep.S8_confounds_and_physio_regressors import (
        run_S8, StepResult,
...
def _check_S8(args):
    from spineprep.S8_confounds_and_physio_regressors import check_S8_confounds_and_physio_regressors
...
def _run_S9(args):
    from spineprep.S9_primary_functional_derivatives import (
        run_S9, StepResult,
...
def _check_S9(args):
    from spineprep.S9_primary_functional_derivatives import check_S9_primary_functional_derivatives
...
def _run_S10(args):
    ...
    """
    from spineprep.S10_qc_aggregation_and_release import run_S10, StepResult
    if args.out is None:
        return StepResult("FAIL", "--out is required")
    return run_S10(
        dataset_key=args.dataset_key if hasattr(args, "dataset_key") else None,
...
def _check_S10(args):
    from spineprep.S10_qc_aggregation_and_release import (
