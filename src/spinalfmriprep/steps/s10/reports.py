...
def _fig_dirs(out_dir: Path, dataset: str, subject: str) -> list[Path]:
    ...
    roots: list[Path] = []
    seen: set[Path] = set()
    for base in (out_dir / "derivatives" / "spinalfmriprep", out_dir / "release"):
        try:
            rb = base.resolve()
        except Exception:
            continue
...
def build_subject_report(
    ...
    # F. Provenance footer
    h.append("<p class='smallnote'>SpinalfMRIprep "
...
def build_group_report(
    ...
    h.append("<p class='smallnote'>Generated by SpinalfMRIprep S10 · QC is advisory.</p>")
    h.append("</div></body></html>")
    out_path.write_text("\n".join(h), encoding="utf-8")
    return out_path
...
def build_overview(
    ...
    h = [f"<!DOCTYPE html><html><head><meta charset='utf-8'>",
         f"<title>SpinalfMRIprep — Release</title><style>{CSS}</style></head>",
         "<body><div class='wrap'>"]
    h.append("<h1>SpinalfMRIprep — Release report</h1>")
    ...
    h.append("<div class='card'><h2>Release artifacts</h2><ul class='links'>"
             ...
             "</ul></div>")
    h.append(f"<p class='smallnote'>SpinalfMRIprep {recipe.get('pipeline_git_describe','?')} · "
