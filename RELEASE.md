...
### 1. Repository
- [ ] Create/settle the public repo at `github.com/SpinalfMRIprep/SpinalfMRIprep`
      (matches `mkdocs.yml` + README/CITATION links).
- [ ] `git remote add origin …` and push `main`.
- [ ] Cut a signed tag: `git tag -a v1.0.0 -m "SpinalfMRIprep 1.0.0" && git push --tags`.
...
- [ ] Confirm the FSL licence terms permit your intended use; keep distribution as
      the **build recipe** (`Dockerfile.spinalfmriprep`) — users build locally.
...
- [ ] Build + host the docs (`mkdocs build`) at `spinalfmriprep.com` or GitHub
...
scripts/container_smoke_test.sh spinalfmriprep:1.0.0 /path/to/bids <subject>
```
