...
def _parse_bids_rel(rel: str) -> Tuple[str,str,str,str]:
    ...
    modality = next((p for p in parts if p in {"anat","func","fmap","dwi","beh"}), "unknown")
    base = parts[-1]
...
def ingest(bids_root: Path, out_dir: Path, hash_large: bool=False) -> Dict[str,int]:
    ...
    with man.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["path","relpath","size","sha256","subject","session","modality","suffix","ext"])
    ...
    return {"total": len(files), "func": func_n, "anat": anat_n, "other": other_n, "manifest": str(man)}
