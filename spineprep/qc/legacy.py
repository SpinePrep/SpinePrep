...
def build_qc_index(
    ...
    if doctor_info:
        ...
        html_parts.append(
            f"      <tr><td>OS</td><td>{plat.get('os', 'unknown')}</td></tr>"
        )
        html_parts.append(
            f"      <tr><td>Python</td><td>{plat.get('python', 'unknown')}</td></tr>"
        )
        ...
        sct = deps.get("sct", {})
        html_parts.append(
            f"      <tr><td>SCT</td><td>{'✓ ' + sct.get('version', '') if sct.get('found') else '✗ Not found'}</td></tr>"
        )
