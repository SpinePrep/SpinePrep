...
class SystemMessage(ApplicationError):
    ...
    def __init__(
        self,
        system_message: nodes.system_message,
        level: int,
    ) -> None:
...
class Reporter:
    ...
    levels: Final[Sequence[str]] = ("DEBUG", "INFO", "WARNING", "ERROR", "SEVERE")
    ...
    def system_message(
        self, level: int, message: str, *children, **kwargs: Any
...
def assemble_option_dict(
    option_list: list[tuple[str, str | None]],
    options_spec: dict[str, Callable[object], Any],
) -> dict[str, Any]:
    """
...
def extract_name_value(line):
    ...
    while line:
        ...
        if line[0] in "'\"":
            ...
            if len(line) > endquote_index + 1 and line[endquote_index + 1].strip():
                raise NameValueError(
                    f'attribute "{attname}" end quote '
...
def new_document(
    source_path: StrPath, settings: Values | None = None
) -> nodes.document:
    """
...
def get_stylesheet_reference(
    settings: Values, relative_to: StrPath | None = None
) -> str:
    """
...
def escape2null(text: str) -> str:
    ...
    while True:
        ...
        if bs_index == -1:
            ...
            return "".join(parts)
        parts.extend((text[start:bs_index], "\x00" + text[bs_index + 1 : bs_index + 2]))
