...
def validate_encoding(
    ...
    if value == "":
        warnings.warn(
            "Input encoding detection will be removed and the "
...
def validate_math_output(
    ...
    if format not in formats:
        raise LookupError(
            f'Unknown math output format: "{value}",\n' f"    choose from {formats}."
        )
    if format == "mathml":
...
def make_paths_absolute(
    pathdict: dict[str, list[StrPath] | StrPath],
    keys: tuple[str],
...
def make_one_path_absolute(base_path: StrPath, path: StrPath) -> str:
    # deprecated, will be removed
    warnings.warn(
        "frontend.make_one_path_absolute() will be removed "
...
class Values(optparse.Values):
    ...
    def __init__(self, defaults: dict[str, Any] | None = None) -> None:
        warnings.warn(
            "frontend.Values class will be removed " "in Docutils 2.0 or later.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class Option(optparse.Option):
    ...
    def __init__(self, *args: str | None, **kwargs: Any) -> None:
        warnings.warn(
            "The frontend.Option class will be removed " "in Docutils 2.0 or later.",
            DeprecationWarning,
            stacklevel=2,
        )
...
class OptionParser(optparse.OptionParser, docutils.SettingsSpec):
    ...
    default_error_encoding: ClassVar[str] = (
        getattr(sys.stderr, "encoding", None) or io._locale_encoding or "ascii"
    )
    ...
    settings_spec = (
        ...
        (
            ...
            (
                ...
                ["--language", "-l"],
                {"dest": "language_code", "default": "en", "metavar": "<tag>"},
            ),
            (
    ...
    def get_config_file_settings(self, config_file: str) -> dict[str, Any]:
        ...
        make_paths_absolute(
            settings.__dict__, self.relative_path_settings, os.path.dirname(config_file)
        )
