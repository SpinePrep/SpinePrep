...
class BasePseudoSection(Directive):
    ...
    def run(self):
        if not isinstance(
            self.state_machine.node, (nodes.document, nodes.section, nodes.sidebar)
        ):
            raise self.error(
...
class LineBlock(Directive):
    ...
    def run(self):
        ...
        (block.source, block.line) = self.state_machine.get_source_and_line(self.lineno)
        self.add_name(block)
        node_list = [block]
...
class MathBlock(Directive):
    ...
    def run(self):
        ...
        for block in content:
            ...
            (node.source, node.line) = self.state_machine.get_source_and_line(
                self.lineno
            )
            self.add_name(node)
            _nodes.append(node)
