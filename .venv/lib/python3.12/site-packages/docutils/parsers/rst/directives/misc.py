...
class Include(Directive):
    ...
    def run(self) -> list:
        ...
        self.clip_options = (
            self.options.get("start-line", None),
        ...
        path = adapt_path(path, self.state.document.current_source, root_prefix)
    ...
    def read_file(self, path: StrPath) -> str:
        ...
        try:
            include_file = io.FileInput(
                source_path=path, encoding=encoding, error_handler=error_handler
            )
    ...
    def custom_parse(self, text: str) -> list:
        ...
        self.state.document.transform_messages.extend(document.transform_messages)
    ...
    def insert_into_input_lines(self, text: str) -> None:
        ...
        """
        source = self.options["source"]
        textlines = statemachine.string2lines(
            text, self.tab_width, convert_whitespace=True
        )
        # Sanity checks:
        ...
        for i, line in enumerate(textlines):
            if len(line) > self.settings.line_length_limit:
                ...
                raise self.warning(
                    f'"{source}": line {line_no} exceeds the' " line-length-limit."
                )
        # circular inclusion
        ...
        if (source, self.clip_options) in include_log:
            ...
            raise self.warning(
                f'circular inclusion in "{self.name}"' f" directive:\n{inclusion_chain}"
            )
...
class Raw(Directive):
    ...
    def run(self):
        ...
        if not settings.raw_enabled or (
            not settings.file_insertion_enabled
        ...
        raw_node = nodes.raw(
            "", text, classes=self.options.get("class", []), **attributes
        )
...
class Role(Directive):
    ...
    def run(self):
        ...
        if self.content_offset > self.lineno or not self.content:
            raise self.error(
                '"%s" directive requires arguments on the first ' "line." % self.name
            )
        args = self.content[0]
    ...
    def parsemeta(self, match):
        ...
        if not indented:
            ...
            msg = self.reporter.info(
                'No content for meta tag "%s".' % name, nodes.literal_block(line, line)
            )
        ...
        for token in tokens[1:]:
            try:
                ...
                msg = self.reporter.error(
                    'Error parsing meta tag attribute "%s": %s.' % (token, detail),
...
class Meta(Directive):
    ...
    def run(self):
        ...
        index = (
            self.state.document.first_child_not_matching_class(
                (nodes.Titular, nodes.meta)
            )
            or 0
        )
