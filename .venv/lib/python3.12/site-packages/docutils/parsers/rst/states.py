...
class RSTStateMachine(StateMachineWS):
    ...
    def run(
        self, input_lines, document, input_offset=0, match_titles=True, inliner=None
    ) -> None:
        """
...
class NestedStateMachine(RSTStateMachine):
    ...
    def __init__(
        self, state_classes, initial_state, debug=False, parent_state_machine=None
    ) -> None:
...
class RSTState(StateWS):
    ...
    def nested_parse(
        ...
        if node == self.state_machine.node and not isinstance(
            node, (nodes.document, nodes.section)
        ):
        ...
        if match_titles:
            ...
            l_start = min(
                len(self.parent.section_hierarchy()), len(self.memo.title_styles)
            )
            my_state_machine.section_level_offset = l_start - l_node

        # run the state machine and populate `node`:
        block_length = len(block)
        my_state_machine.run(block, input_offset, self.memo, node, match_titles)
    ...
    def check_subsection(self, source, style, lineno) -> bool:
        ...
        oldlevel = len(parent_sections) + self.state_machine.section_level_offset
        # new section level:
        ...
        if newlevel <= oldlevel:
            ...
            try:
                ...
                styles = " ".join("/".join(style) for style in title_styles)
                details = (
                    f"The parent of level {newlevel} sections cannot"
...
class Inliner:
    ...
    def init_customizations(self, settings) -> None:
        ...
        self.patterns = Struct(
            ...
            email=re.compile(self.email_pattern % args + "$", re.VERBOSE),
            uri=re.compile(
                (
                    r"""
...
class Body(RSTState):
    ...
    def enumerator(self, match, context, next_state):
        ...
        (enumlist.source, enumlist.line) = self.state_machine.get_source_and_line()
    ...
    def parse_enumerator(self, match, expected_sequence=None):
        ...
        text = groupdict[format][
            ...
            .start : self.enum.formatinfo[format]  # noqa: E203,E501
            .end
        ]
        if text == "#":
            sequence = "#"
    ...
    def line_block(self, match, context, next_state):
        ...
        self.parent += block
        lineno = self.state_machine.abs_line_number()
        (block.source, block.line) = self.state_machine.get_source_and_line(lineno)
    ...
    def line_block_line(self, match, lineno):
        ...
        (line.source, line.line) = self.state_machine.get_source_and_line(lineno)
    ...
    def isolate_grid_table(self):
        ...
        try:
            ...
            messages.append(
                self.reporter.error("Unexpected indentation.", source=src, line=srcline)
            )
            blank_finish = False
        block.disconnect()
...
class SubstitutionDef(Body):
    ...
    patterns = {
        "embedded_directive": re.compile(r"(%s)::( +|$)" % Inliner.simplename),
        "text": r"",
    }
...
class Text(RSTState):
    ...
    def literal_block(self):
        ...
        (literal_block.source, literal_block.line) = (
    ...
    def definition_list_item(self, termline):
        ...
        (dl_item.source, dl_item.line) = self.state_machine.get_source_and_line(
            dt_lineno
        )
...
class QuotedLiteralBlock(RSTState):
    ...
    def indent(self, match, context, next_state):
        ...
        self.messages.append(
            self.reporter.error(
                "Unexpected indentation.", line=self.state_machine.abs_line_number()
            )
        )
        self.state_machine.previous_line()
        raise EOFError
