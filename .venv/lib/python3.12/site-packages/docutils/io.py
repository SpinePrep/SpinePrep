...
class Input(TransformSpec):
    ...
    def decode(self, data: str | bytes) -> str:
        ...
        if not self.encoding and encoding_candidates[0] != "utf-8":
            warnings.warn(
                "Input encoding auto-detection will be removed and "
    ...
    coding_slug: ClassVar[re.Pattern[bytes]] = re.compile(rb"coding[:=]\s*([-\w.]+)")
...
class Output(TransformSpec):
    ...
    def encode(self, data: str | bytes) -> str | bytes:
        ...
        if self.encoding and self.encoding.lower() == "unicode":
            assert isinstance(data, str), (
                'output encoding is "unicode" ' "but `data` is no `str` instance"
            )
            return data
...
class FileOutput(Output):
    ...
    def __init__(
        ...
        """
        super().__init__(destination, destination_path, encoding, error_handler)
        self.opened = True
        self.autoclose = autoclose
        if handle_io_errors is not None:
            warnings.warn(
                'io.FileOutput: init argument "handle_io_errors" '
...
class BinaryFileOutput(FileOutput):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        warnings.warn(
            '"BinaryFileOutput" is obsoleted by "FileOutput"'
