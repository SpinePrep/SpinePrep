...
class Publisher:
    ...
    def set_components(self, reader_name, parser_name, writer_name) -> None:
        warnings.warn(
            "`Publisher.set_components()` will be removed in "
    ...
    def process_programmatic_settings(
        ...
        if self.settings is None:
            ...
            self.get_settings(
                settings_spec=settings_spec, config_section=config_section, **defaults
            )
    ...
    def set_destination(
        ...
        if destination_path is not None:
            ...
            if getattr(self.settings, "output", None) and not self.settings.output_path:
                self.settings.output_path = self.settings.output
            if (
                self.settings.output_path
                and self.settings._destination
        ...
        self.settings._destination = self.settings.output = self.settings.output_path
    ...
    def debugging_dumps(self) -> None:
        ...
        if self.settings.dump_transforms:
            ...
            print(
                " (priority, transform class, pending node details, " "keyword args)",
                file=self._stderr,
            )
            print(
                pprint.pformat(
        ...
        if self.settings.dump_pseudo_xml:
            ...
            print(
                self.document.pformat().encode("raw_unicode_escape"), file=self._stderr
            )
    ...
    def prompt(self) -> None:
        ...
        try:
            ...
            pass
        print(
            f"Docutils {__version__} <https://docutils.sourceforge.io>\n"
...
def publish_cmdline_to_binary(
    ...
    """
    warnings.warn(
        '"publish_cmdline_to_binary()" is obsoleted'
    ...
    publisher = Publisher(
        reader, parser, writer, settings=settings, destination_class=destination_class
    )
...
def _name_arg_warning(*name_args) -> None:
    for component, name_arg in zip(("reader", "parser", "writer"), name_args):
        if name_arg is not None:
            warnings.warn(
                f'Argument "{component}_name" will be removed in '
...
def rst2something(writer, documenttype, doc_path="") -> None:
    ...
    description = (
        ...
        f"<https://docutils.sourceforge.io/docs/{doc_path}>.  " + default_description
    )
    try:
