...
class Element(Node):
    ...
    def first_child_matching_class(
        self,
        childclass: type[Element] | type[Text] | tuple[type[Element] | type[Text], ...],
        start: int = 0,
        end: int = sys.maxsize,
    ) -> int | None:
        """
    ...
    def validate_attributes(self) -> None:
        ...
        if messages:
            raise ValidationError(
                f"Element {self.starttag()} invalid:\n  " + "\n  ".join(messages),
    ...
    def validate_content(
        ...
        for category, quantifier in model:
            if not isinstance(child, category):
                if quantifier in (".", "+"):
                    raise ValidationError(
                        self._report_child(child, category), problematic_element=child
                    )
    ...
    def _report_child(
        ...
        if isinstance(child, Text):
            return (
                f"{msg}  Expecting child of type <{_type}>, "
                f'not text data "{child.astext()}".'
            )
        return f"{msg}  Expecting child of type <{_type}>, " f"not {child.starttag()}."
...
class subtitle(Titular, PreBibliographic, SubStructural, TextElement):
    ...
    def validate_position(self) -> None:
        ...
        if self.parent and self.parent.index(self) == 0:
            raise ValidationError(
                f"Element {self.parent.starttag()} invalid:"
...
class decoration(PreBibliographic, SubStructural, Element):
    ...
    content_model: Final = (
        (header, "?"),  # Empty element doesn't make sense,
...
class document(Root, Element):
    ...
    def set_duplicate_name_id(
        ...
        if old_id is not None and (
            "refname" in node
            and node["refname"] == old_node.get("refname")
            or "refuri" in node
        ...
        if level:
            ...
            msg = self.reporter.system_message(
                level, s, backrefs=backrefs, base_node=node
            )
...
def split_name_list(s: str) -> list[str]:
    ...
    return [name.replace("\x00\x00", " ").replace("\x00", "") for name in names]
...
def parse_measure(
    measure: str, unit_pattern: str = "[a-zA-Zµ]*|%?"
) -> tuple[int | float, str]:
