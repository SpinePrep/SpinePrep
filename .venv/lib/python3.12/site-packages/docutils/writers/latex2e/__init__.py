# $Id: __init__.py 10231 2025-09-11 14:16:33Z milde $
...
class SortableDict(dict):  # NoQA: FURB189
    ...
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "`writers.latex2e.SortableDict` is obsolete"
...
PreambleCmds.table_columnwidth = (
    r"\newcommand{\DUcolumnwidth}[1]" r"{\dimexpr#1\DUtablewidth-2\tabcolsep\relax}"
)
...
class DocumentClass:
    ...
    def __init__(self, document_class, with_part=False) -> None:
        ...
        if (
            self.document_class.endswith("book")
            or self.document_class.endswith("report")
            or self.document_class in ("ctexrep", "memoir", "mwbk", "mwrep", "scrreprt")
        ):
...
class Table:
        ...
        if not self.colwidths_auto:
            if self.borders == "standard" and not self.legacy_column_widths:
                opening.append(
                    ...
                    r"{\dimexpr%s-%i\arrayrulewidth\relax}%%" % (width, nr_of_cols + 1)
                )
            else:
    ...
    def get_colspecs(self, node):
        ...
        if self.colwidths_auto:
            ...
            self._colwidths = [
                (factor * (node.propwidth() + 1) / width) + 0.005
            ...
            self._colwidths = [
                (node.propwidth() + allowance) / norm_length for node in self._col_specs
            ]
...
class LaTeXTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document, babel_class=Babel) -> None:
        ...
        if settings.literal_block_env:
            (_none, self.literal_block_env, self.literal_block_options, _none) = (
        ...
        # Document parts
        self.head_prefix = [
            f"\\documentclass[{self.documentoptions}]" f"{{{settings.documentclass}}}"
        ]
        ...
        if not settings.documentclass.startswith("scr") and (
            "DIV" in self.documentoptions or "BCOR" in self.documentoptions
        ):
        ...
        if self.fallback_stylesheet:
            ...
            if settings.legacy_class_functions:
                ...
                self.fallbacks["_docutils.sty"] = r"\usepackage{docutils}[2025-08-06]"
    ...
    def ids_to_labels(
        self, node, set_anchor=True, protect=False, newline=False, pre_nl=False
    ) -> list[str]:
    ...
    def visit_docinfo_item(self, node, name=None) -> None:
        ...
        if name is not None:
            warnings.warn(
                'visit_docinfo_item(): argument "name" is obsolete'
                " and will be removed in Docutils 0.24",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def depart_document(self, node) -> None:
        ...
        if self.document.get("title", ""):
            self.pdfinfo.insert(
                0, "  pdftitle={%s}," % self.encode(self.document.get("title", ""))
            )
        if self.pdfauthor:
        ...
        self.requirements = [
            self.requirements[key] for key in sorted(self.requirements.keys())
        ]
    ...
    def to_latex_length(self, length_str, node=None) -> str:
        ...
        if not hasattr(PreambleCmds, unit):
            self.warn(
                f'Unit "{unit}" not supported by LaTeX.\n'
    ...
    def visit_inline(self, node) -> None:
        ...
        set_anchor = not (
            isinstance(node.parent, anchor_nodes) or isinstance(node, anchor_nodes)
        )
    ...
    def visit_contents(self, node):
        ...
        try:
            ...
            self.warn(
                'Setting "use_latex_toc" is True but "contents" details '
