...
class Writer(writers._html_base.Writer):
    ...
    default_stylesheet_dirs = [
        ...
        os.path.abspath(
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "html5_polyglot")
        ),  # for math.css
    ]
...
class HTMLTranslator(writers._html_base.HTMLTranslator):
    ...
    def visit_citation(self, node) -> None:
        self.body.append(
            self.starttag(
                node, "table", CLASS="docutils citation", frame="void", rules="none"
            )
        )
        self.body.append(
    ...
    def visit_citation_reference(self, node) -> None:
        ...
        self.body.append(
            self.starttag(
                node, "a", suffix="[", href=href, classes=["citation-reference"]
            )
        )
    ...
    def visit_field_list(self, node) -> None:
        ...
        self.body.append(
            self.starttag(
                node, "table", frame="void", rules="none", CLASS="docutils field-list"
            )
        )
        self.body.append(
    ...
    def visit_footnote(self, node) -> None:
        self.body.append(
            self.starttag(
                node, "table", CLASS="docutils footnote", frame="void", rules="none"
            )
        )
        self.body.append(
            '<colgroup><col class="label" /><col /></colgroup>\n'
        ...
        if self.settings.footnote_backlinks and backrefs:
            if len(backrefs) == 1:
                ...
                for i, backref in enumerate(backrefs, 1):
                    backlinks.append(
                        '<a class="fn-backref" href="#%s">%s</a>' % (backref, i)
                    )
    ...
    def visit_footnote_reference(self, node) -> None:
        ...
        self.body.append(
            self.starttag(node, "a", suffix, CLASS="footnote-reference", href=href)
        )
    ...
    def visit_image(self, node) -> None:
        ...
        if "scale" in node:
            if (
                ...
                try:
                    ...
                    self.document.reporter.warning(f"Problem reading image file: {e}")
                else:
        ...
        if ext in self.object_image_types:
            ...
            self.body.append(
                self.starttag(node, "object", "", **atts)
                + node.get("alt", uri)
                + "</object>"
                + suffix
            )
    ...
    def visit_label(self, node) -> None:
        self.body.append(
            self.starttag(node, "td", "%s[" % self.context.pop(), CLASS="label")
        )
    ...
    def visit_option_list(self, node) -> None:
        self.body.append(
            self.starttag(
                node, "table", CLASS="docutils option-list", frame="void", rules="none"
            )
        )
        self.body.append(
            '<col class="option" />\n'
    ...
    def visit_term(self, node) -> None:
        self.body.append(
            self.starttag(
                node, "dt", "", classes=node.parent["classes"], ids=node.parent["ids"]
            )
        )
