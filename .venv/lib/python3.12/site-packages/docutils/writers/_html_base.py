...
class HTMLTranslator(writers.DoctreeTranslator):
    ...
    def __init__(self, document) -> None:
        ...
        self.image_loading = getattr(settings, "image_loading", _image_loading_default)
        ...
        (self.math_output, self.math_options) = self.settings.math_output
        ...
        self.stylesheet = [
            self.stylesheet_call(path) for path in utils.get_stylesheet_list(settings)
        ]
        self.title = []
        self.subtitle = []
        if settings.xml_declaration:
            self.head_prefix.append(utils.xml_declaration(settings.output_encoding))
            self.html_prolog.append(
                utils.xml_declaration("%s")
            )  # encoding not interpolated
        if settings.output_encoding and settings.output_encoding.lower() != "unicode":
    ...
    def prepare_svg(self, code: str, node: nodes.Element, atts: dict) -> str:
        ...
        if "style" in atts:
            ...
            svg.set(
                "style", " ".join(f"{k}: {v};" for k, v in declaration_dict.items())
            )
        ...
        if ids:
            ...
            for id in ids[1:]:
                ...
                if empty or isinstance(
                    node, (nodes.Sequential, nodes.docinfo, nodes.table)
                ):
    ...
    def visit_citation(self, node) -> None:
        ...
        self.body.append(
            self.starttag(node, "div", classes=[node.tagname], role="doc-biblioentry")
        )
    ...
    def depart_colspec(self, node) -> None:
        ...
        for node in self.colspecs:
            ...
            self.body.append(self.emptytag(node, "col", style=f"width: {colwidth:.1%}"))
    ...
    def visit_docinfo_item(self, node, name, meta=True) -> None:
        if meta:
            self.meta.append(
                f'<meta name="{name}" ' f'content="{self.attval(node.astext())}" />\n'
            )
        self.body.append(
            f'<dt class="{name}">{self.language.labels[name]}'
    ...
    def visit_doctest_block(self, node) -> None:
        self.body.append(
            self.starttag(node, "pre", suffix="", classes=["code", "python", "doctest"])
        )
    ...
    def depart_document(self, node) -> None:
        ...
        if (
            self.settings.output_encoding  # may be None
    ...
    def visit_footnote(self, node) -> None:
        ...
        self.body.append(
            self.starttag(
                node, "aside", classes=[node.tagname, label_style], role="doc-footnote"
            )
        )
    ...
    def depart_label(self, node) -> None:
        ...
        if len(backrefs) > 1:
            backlinks = [
                '<a role="doc-backlink" href="#%s">%s</a>' % (ref, i)
    ...
    def visit_math(self, node):
        ...
        if format == "html":
            ...
            try:
                ...
                self.messages.append(
                    self.document.reporter.warning(err, *details, base_node=node)
                )
        ...
        if tag:
            self.body.append(
                self.starttag(
                    node, tag, suffix=suffix, classes=self.math_tags[format][2]
                )
            )
