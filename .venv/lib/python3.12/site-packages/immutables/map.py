    ...
    def assoc(self, shift, hash, key, val, mutid):
        ...
        if self.bitmap & bit:
            ...
            if key_or_null is _NULL:
                sub_node, added = val_or_node.assoc(shift + 5, hash, key, val, mutid)
            ...
            if existing_key_hash == hash:
                ...
                sub_node, _ = sub_node.assoc(
                    shift + 5, existing_key_hash, key_or_null, val_or_node, mutid
                )
                sub_node, _ = sub_node.assoc(shift + 5, hash, key, val, mutid)
            ...
            if mutid and mutid == self.mutid:
                ...
                return (
                    BitmapNode(2 * (n + 1), self.bitmap | bit, new_array, mutid),
                    True,
                )
    ...
    def without(self, shift, hash, key, mutid):
        ...
        if key_or_null is _NULL:
            ...
            if res is W_EMPTY:
                ...
                if (
                    type(sub_node) is BitmapNode
                    and sub_node.size == 2
                    and sub_node.array[0] is not _NULL
                ):
...
class CollisionNode:
        ...
        if hash == self.hash:
            ...
            if key_idx == -1:
                ...
                if mutid and mutid == self.mutid:
                    ...
                    new_node = CollisionNode(self.size + 2, hash, new_array, mutid)
            ...
            new_node = BitmapNode(2, map_bitpos(self.hash, shift), [_NULL, self], mutid)
    ...
    def without(self, shift, hash, key, mutid):
        ...
        if new_size == 2:
            ...
            new_node = BitmapNode(2, map_bitpos(hash, shift), new_array, mutid)
            return W_NEWNODE, new_node
        ...
        if mutid and mutid == self.mutid:
            ...
            new_node = CollisionNode(self.size - 2, self.hash, new_array, mutid)
            return W_NEWNODE, new_node
    ...
    def dump(self, buf, level):  # pragma: no cover
        pad = "    " * (level + 1)
        buf.append(pad + "CollisionNode(size={} id={:0x}):".format(self.size, id(self)))
        ...
        while True:
            ...
            try:
                ...
                raise TypeError(
                    ...
                    "sequence element #{} to a sequence".format(i)
                ) from None
            key, val, *r = tup
            if r:
                raise ValueError(
...
class MapMutation:
    ...
    def __delitem__(self, key):
        ...
        res, new_root = self.__root.without(0, map_hash(key), key, self.__mutid)
        if res is W_EMPTY:
            self.__count = 0
    ...
    def __setitem__(self, key, val):
        ...
        self.__root, added = self.__root.assoc(0, map_hash(key), key, val, self.__mutid)
