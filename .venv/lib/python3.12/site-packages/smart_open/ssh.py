...
def open_uri(uri, mode, transport_params):
    ...
    final_params = {
        **parsed_uri,
        **kwargs,
    }  # transport_params takes precedence over uri
...
def _maybe_fetch_config(
    host, username=None, password=None, port=None, connect_kwargs=None
):
    ...
    for attempt in range(attempts):
        try:
            ...
            if not ssh.get_transport().active:
                ssh.close()
                ssh = _SSH[key] = _connect_ssh(
                    host, user, port, password, connect_kwargs
                )
        except KeyError:
