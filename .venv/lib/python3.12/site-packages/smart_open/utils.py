...
def check_kwargs(kallable, kwargs):
    ...
    if unsupported_keywords:
        logger.warning(
            "ignoring unsupported keyword arguments: %r", unsupported_keywords
        )
...
def safe_urlsplit(url):
    ...
    placeholder = None
    if (
        sr.scheme in WORKAROUND_SCHEMES
        and "?" in url
        and QUESTION_MARK_PLACEHOLDER not in url
    ):
        #
...
class FileLikeProxy(wrapt.ObjectProxy):
    ...
    def close(self):
        try:
            ...
            if (
                self.__inner != self.__wrapped__
            ):  # Don't close again if inner and wrapped are the same
                self.__inner.close()
