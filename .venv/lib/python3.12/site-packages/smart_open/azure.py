...
def open(
    ...
    if mode == smart_open.constants.READ_BINARY:
        ...
        raise NotImplementedError(
            "Azure Blob Storage support for mode %r not implemented" % mode
        )
    ...
    def _download_blob_chunk(self, size):
        if self._size == self._position:
            ...
            stream = self._blob.download_blob(
                offset=self._position, max_concurrency=self._concurrency
            )
        else:
            stream = self._blob.download_blob(
...
class Reader(io.BufferedIOBase):
    ...
    _blob = (
        None  # so `closed` property works in case __init__ fails and __del__ is called
    )
