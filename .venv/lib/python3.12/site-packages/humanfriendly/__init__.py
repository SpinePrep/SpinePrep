...
length_size_units = (
    dict(prefix="nm", divider=1e-09, singular="nm", plural="nm"),
...
def parse_size(size, binary=False):
    ...
    if tokens and isinstance(tokens[0], numbers.Number):
        ...
        normalized_unit = (
            tokens[1].lower() if len(tokens) == 2 and is_string(tokens[1]) else ""
        )
...
def format_length(num_metres, keep_width=False):
    ...
    for unit in reversed(length_size_units):
        if num_metres >= unit["divider"]:
            number = round_number(
                float(num_metres) / unit["divider"], keep_width=keep_width
            )
...
def parse_date(datestring):
    ...
    try:
        ...
        msg = (
            "Invalid date! (expected 'YYYY-MM-DD' or 'YYYY-MM-DD HH:MM:SS' but got: %r)"
        )
