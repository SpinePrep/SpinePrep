...
def ansi_style(**kw):
    ...
    **Support for 24-bit colors**
    ...
    sequences = [
        ANSI_TEXT_STYLES[k] for k, v in kw.items() if k in ANSI_TEXT_STYLES and v
    ]
    ...
    for color_type in "color", "background":
        ...
        if isinstance(color_value, (tuple, list)):
            ...
            sequences.extend(
                (39 if color_type == "background" else 38, 5, int(color_value))
            )
        elif color_value:
...
def find_terminal_size_using_ioctl(stream):
    ...
    h, w, hp, wp = struct.unpack(
        "HHHH", fcntl.ioctl(stream, termios.TIOCGWINSZ, struct.pack("HHHH", 0, 0, 0, 0))
    )
    return h, w
