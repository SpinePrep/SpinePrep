...
from humanfriendly.deprecation import (
    DeprecationProxy,
    define_aliases,
    deprecated_args,
    get_aliases,
...
class HumanFriendlyTestCase(TestCase):
    ...
    def test_case_insensitive_dict(self):
        ...
        assert (
            len(
                CaseInsensitiveDict(
                    [("one", True), ("ONE", False)], one=False, two=True
                )
            )
            == 2
        )
        ...
        assert (
            len(CaseInsensitiveDict(dict(one=True, ONE=False), one=False, two=True))
            == 2
        )
    ...
    def test_compact(self):
        ...
        """,
            ...
            == "More readable template notation"
        )
    ...
    def test_format_timespan(self):
        ...
        assert (
            "6 years, 5 weeks, 4 days, 3 hours, 2 minutes and 500 milliseconds"
            == format_timespan(
                year * 6 + week * 5 + day * 4 + hour * 3 + minute * 2 + 0.5,
                detailed=True,
            )
        )
        assert "1 year, 2 weeks and 3 days" == format_timespan(
            year + week * 2 + day * 3 + hour * 12
        )
        ...
        assert "1 minute, 1 second and 100 milliseconds" == format_timespan(
            61.10, detailed=True
        )
        ...
        assert "10 seconds and 15 milliseconds" == format_timespan(
            10.015, detailed=True
        )
        assert "1 microsecond and 50 nanoseconds" == format_timespan(
            0.00000105, detailed=True
        )
    ...
    def test_pretty_tables(self):
        ...
        data = [["1", "2", "3"], ["a", "b", "c"]]
        assert (
            ansi_strip(format_pretty_table(data, column_names))
            == dedent(
                """
            ---------------------
            | One | Two | Three |
    ...
    def test_robust_tables(self):
        ...
        data = [["1", "2", "3"], ["a", "b", "c"]]
        assert (
            ansi_strip(format_robust_table(data, column_names))
            == dedent(
                """
            --------
            One: 1
    ...
    def test_smart_tables(self):
        ...
        data = [["1", "2", "3"], ["a", "b", "c"]]
        assert (
            ansi_strip(format_smart_table(data, column_names))
            == dedent(
                """
            ---------------------
            | One | Two | Three |
    ...
    def test_concatenate(self):
        ...
        assert (
            concatenate(["one", "two", "three"], conjunction="or")
            == "one, two or three"
        )
        # Test the 'serial_comma' option.
        assert (
            concatenate(["one", "two", "three"], serial_comma=True)
            == "one, two, and three"
        )
    ...
    def test_timer(self):
        ...
        for seconds, text in (
            ...
            self.assertEqual(
                round_number(t.elapsed_time, keep_width=True), "%i.00" % seconds
            )
    ...
    def test_spinner(self):
        ...
        stream = StringIO()
        spinner = Spinner(
            label="test spinner", total=4, stream=stream, interactive=True
        )
    ...
    def test_prompt_for_confirmation(self):
        ...
        for default_choice in True, False:
            with PatchedAttribute(prompts, "interactive_prompt", lambda p: ""):
                assert (
                    prompt_for_confirmation("Are you sure?", default=default_choice)
                    is default_choice
                )
        ...
        with PatchedAttribute(prompts, "interactive_prompt", lambda p: "y"):
            for default_value, expected_text in (
                (True, "Y/n"),
                (False, "y/N"),
                (None, "y/n"),
            ):
                with CaptureOutput(merged=True) as capturer:
                    assert (
                        prompt_for_confirmation("Are you sure?", default=default_value)
                        is True
                    )
    ...
    def test_prompt_for_input(self):
        ...
        with open(os.devnull) as handle:
            with PatchedAttribute(sys, "stdin", handle):
                ...
                assert (
                    prompt_for_input("What is your quest?", default=default_value)
                    == default_value
                )
    ...
    def test_cli(self):
        ...
        returncode, output = run_cli(
            main, "--format-size=%i" % random_byte_count, "--binary"
        )
        ...
        returncode, output = run_cli(
            main, "--format-table", "--delimiter=\t", input="1\t2\t3\n4\t5\t6\n7\t8\t9"
        )
        assert (
            output.strip()
            == dedent(
        ...
        returncode, output = run_cli(
            main, "--run-command", "bash", "-c", "sleep 2 && exit 42"
        )
    ...
    def test_ansi_style(self):
        ...
        assert ansi_style(color=(255, 255, 255)) == "%s38;2;255;255;255%s" % (
            ANSI_CSI,
            ANSI_SGR,
        )
        assert ansi_style(background=(50, 100, 150)) == "%s48;2;50;100;150%s" % (
            ANSI_CSI,
            ANSI_SGR,
        )
        with self.assertRaises(ValueError):
    ...
    def test_html_to_ansi(self):
        ...
        # Hyperlinks.
        assert (
            html_to_ansi('<a href="https://python.org">python.org</a>')
        ...
        assert (
            html_to_ansi(
                '<a href="mailto:peter@peterodding.com">peter@peterodding.com</a>'
            )
            == "\x1b[0m\x1b[4;94mpeter@peterodding.com\x1b[0m"
        )
        # Bold text.
        assert (
            html_to_ansi("Let's try <b>bold</b>")
            == "Let's try \x1b[0m\x1b[1mbold\x1b[0m"
        )
        assert (
            html_to_ansi('Let\'s try <span style="font-weight: bold">bold</span>')
            == "Let's try \x1b[0m\x1b[1mbold\x1b[0m"
        )
        # Italic text.
        assert (
            html_to_ansi("Let's try <i>italic</i>")
            == "Let's try \x1b[0m\x1b[3mitalic\x1b[0m"
        )
        assert (
            html_to_ansi('Let\'s try <span style="font-style: italic">italic</span>')
            == "Let's try \x1b[0m\x1b[3mitalic\x1b[0m"
        )
        # Underlined text.
        assert (
            html_to_ansi("Let's try <ins>underline</ins>")
            == "Let's try \x1b[0m\x1b[4munderline\x1b[0m"
        )
        assert (
            html_to_ansi(
                'Let\'s try <span style="text-decoration: underline">underline</span>'
            )
            == "Let's try \x1b[0m\x1b[4munderline\x1b[0m"
        )
        # Strike-through text.
        assert (
            html_to_ansi("Let's try <s>strike-through</s>")
            == "Let's try \x1b[0m\x1b[9mstrike-through\x1b[0m"
        )
        assert (
            html_to_ansi(
                'Let\'s try <span style="text-decoration: line-through">strike-through</span>'
            )
            == "Let's try \x1b[0m\x1b[9mstrike-through\x1b[0m"
        )
        # Pre-formatted text.
        assert (
            html_to_ansi("Let's try <code>pre-formatted</code>")
            == "Let's try \x1b[0m\x1b[33mpre-formatted\x1b[0m"
        )
        # Text colors (with a 6 digit hexadecimal color value).
        assert (
            html_to_ansi('Let\'s try <span style="color: #AABBCC">text colors</s>')
            == "Let's try \x1b[0m\x1b[38;2;170;187;204mtext colors\x1b[0m"
        )
        # Background colors (with an rgb(N, N, N) expression).
        assert (
            html_to_ansi(
                'Let\'s try <span style="background-color: rgb(50, 50, 50)">background colors</s>'
            )
            == "Let's try \x1b[0m\x1b[48;2;50;50;50mbackground colors\x1b[0m"
        )
        # Line breaks.
        assert (
            html_to_ansi("Let's try some<br>line<br>breaks")
            == "Let's try some\nline\nbreaks"
        )
    ...
    def test_generate_warning(self):
        ...
        with CaptureOutput(merged=False) as capturer:
            ...
            self.assertEqual([], capturer.stdout.get_lines())
            self.assertEqual(
                [ansi_wrap(text, color="red")],
                self.ignore_coverage_warning(capturer.stderr),
            )
    ...
    def test_get_pager_command(self):
        ...
        assert "--RAW-CONTROL-CHARS" in get_pager_command(
            ansi_wrap("Usage message", bold=True)
        )
        ...
        for pager in "cat", "less":
            ...
            try:
                ...
                if pager == "less":
                    ...
                    assert not any(
                        opt in command_line for opt in options_specific_to_less
                    )
            finally:
                if original_pager is not None:
                    # Restore the original $PAGER value.
    ...
    def test_render_usage(self):
        ...
        assert (
            render_usage("Usage: some-command WITH ARGS")
            == "**Usage:** `some-command WITH ARGS`"
        )
        assert render_usage("Supported options:") == "**Supported options:**"
        assert "code-block" in render_usage(
            dedent(
                """
    ...
    def test_alias_proxy_sphinx_integration(self):
        ...
        deprecation_note_callback(
            app=None, what=None, name=None, obj=module, options=None, lines=lines
        )
    ...
    def test_sphinx_customizations(self):
        ...
        assert (
            deprecation_note_callback in fake_app.callbacks["autodoc-process-docstring"]
        )
