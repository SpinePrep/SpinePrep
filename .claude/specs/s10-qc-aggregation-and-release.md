---
status: approved
supersedes: private/SPEC/S10_qc_aggregation_and_reports.md
---

# Scope Spec: S10 QC Aggregation + Release Readiness v2

> **Update 2026-06-11 — former S10 (ROI/connectivity) removed; this step
> aggregates S1–S9 only.** The former S10 (ROI timeseries / hemicord
> connectivity / reliability) was removed from the active pipeline on
> 2026-06-11 as analyst-owned analysis (its own spec is `status: deferred`),
> and its step number was reused for this QC aggregation & release step. As a
> result the **Cohort FC summary** deliverable (item 7) and the
> `max_condition_number` participants column are **removed** from the shipped
> step, and the former-S10 hemicord-TSV data source no longer applies. The
> references below that mention the former S10 (ROI/connectivity), the FC
...
7. ~~**Cohort FC summary**~~ — **REMOVED 2026-06-11 with the former S10.** This
   ...
   Fisher-z matrix + consistency map). Since the former S10 is no longer in the
   active pipeline, this step no longer emits a cohort FC summary. Functional
...
### Spec housekeeping
- Mark `private/SPEC/S10_qc_aggregation_and_reports.md` → `superseded`.

## Inputs
- All `logs/S{N}_*/<dataset>/qc.json` files for N in 1..**9** (walked via chain symlinks). (Former S10 removed 2026-06-11.)
- Per-run derivative artifacts:
  - S9: `*_desc-tsnr_per_level.tsv`, `*_desc-tsnr_native.nii.gz`.
  - ~~Former S10: `*_desc-hemicord_pearson_connectivity.tsv`, `*_desc-hemicord_fisherz_connectivity.tsv`.~~ — removed with the former S10.
...
6. ~~Cohort FC summary (item 7) computed across all subjects with hemicord FC matrices.~~ — removed with the former S10 (2026-06-11); no longer an acceptance criterion.
...
8. Re-running S10 on same chain produces byte-identical `metrics_index.jsonl` (deterministic).
...
5. Implement 14 generators in `process.py`:
   ...
   - ~~`_build_cohort_fc_summary(s10_hemicord_tsvs)`~~ — removed with the former S10 (2026-06-11).
...
7. Run S10 on the reg chain; verify `release_report.html` opens; verify each item.
...
| D4 | Cohort FC summary is descriptive aggregation only (no p-values) | Statistical inference is analyst territory; S10 stays a pipeline preprocessing aggregator. |
...
| D10 | Idempotent + deterministic output | Required for reproducibility claims. No randomness anywhere in S10. |
...
| 8 | Full cohort = deliverable | ✅ **S10 is the release deliverable itself** by design. The CITATION.cff, methods_manifest.tex, dataset_description.json are all paper-time outputs. |
| 9 | Reproducible | ✅ S10 *emits* the reproducibility receipt (SCT/FSL/Python tool versions + policy SHA256 + git SHA). The whole step exists to make the pipeline reproducible by third parties. |
| 10 | Heterogeneity is the test | ✅ The **per-vertebral-level coverage matrix** + **cohort tSNR heatmap** are the heterogeneity surfaces — they show directly which levels are covered across the 5 reg datasets so the analyst stratifies rather than pools blindly. (The earlier headline finding `cohort_fc_n_common_rois: 1` came from the now-removed former-S10/FC-summary path; the coverage matrix carries the same message without it.) |

## Why S10 doesn't have per-run reportlets

S10 is **global / cross-dataset**, not per-run. The dashboard layer
...
| ~~`cohort_fc_n_common_rois`~~ | **Removed 2026-06-11 with the former S10.** Was the number of hemicord ROIs with usable timeseries across all aggregated runs — the cohort-FC summary's usable axis. No longer computed; heterogeneity is now read off the coverage matrix + tSNR heatmap. |
| `n_subjects_aggregated` / `n_runs_aggregated` / `n_datasets` | Cohort scale at this run; reproducibility receipt. |

## Decision: no code change

S10 received the deepest design audit in this cycle (3 rounds:
...
step in the pipeline now has a principles audit, S10 should too.

## Remaining gaps (acceptable / deferred)

- *(Historical, no longer applicable.)* The cohort FC summary and its
  `cohort_fc_n_common_rois` axis were removed with the former S10 on 2026-06-11.
  ...
  the underlying level-coverage limitation, which is the part this step
  keeps.
