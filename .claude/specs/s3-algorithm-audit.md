...
|---|---|---|
| **S3.1** Dummy drop + cord localization + drift gate | Trim N initial frames, build fast median funcref, segment cord via `sct_deepseg seg_sc_contrast_agnostic`, reject runs where the cord seg leaks into brain | `sct_deepseg` (SCT ≥ 7.0) |
| **S3.2** Frame-metrics + outlier flagging + robust funcref | Compute mask-aware DVARS + refRMS within the cord ROI per frame, flag outliers via boxplot cutoff, build median funcref over non-outlier frames | NumPy + nibabel |
...
The drift gate is a SpinalfMRIprep contribution. We documented it as
...
| **refRMS within cord mask** | RMS of (frame − reference) | Standard fMRIPrep / MRIQC; Kaptan 2023 / Dabbagh 2024 cord | ✅ standard |
...
| **Combination rule** | DVARS OR refRMS above threshold | Standard dual-criterion (Power 2014 brain + Kaptan 2023 cord) | ✅ standard |
...
| Volume-level FD (frame-wise displacement) | **deferred to S4** | S4 motion correction produces FD from its rigid params; S3's mask-aware DVARS + refRMS are the cord-level analogs. ✅ correct separation |
...
| "refRMS as standard cord metric" | ✅ | Kaptan 2023 / Dabbagh 2024 |
...
| "drift gate is literature-backed" | ❌ | **Pipeline-specific innovation; honest documentation in this audit doc.** |
...
- ⚠️ Choices that deviate intentionally and defensibly: Tukey
  ...
  35 mm), drift gate (pipeline-specific).
