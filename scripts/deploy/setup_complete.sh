#!/bin/bash
# Complete setup script for SpinalfMRIprep canonical dashboard server
...
echo "=========================================="
echo "SpinalfMRIprep Dashboard Server - Complete Setup"
echo "=========================================="
echo ""
...
if systemctl is-active --quiet spinalfmriprep-dashboard.service; then
    echo "[2/4] ✓ Service is running"
else
    echo "[2/4] ✗ Service failed to start. Check logs:"
    echo "      sudo journalctl -u spinalfmriprep-dashboard.service -n 20"
    exit 1
fi
echo ""
...
TS_HOSTNAME=$(tailscale status 2>&1 | grep -E "^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\s+spinalfmriprep-dash" | awk '{print $2}' || echo "")

if [[ "$TS_HOSTNAME" == "spinalfmriprep-dash" ]]; then
    echo "[3/4] ✓ Hostname already set to spinalfmriprep-dash"
elif [[ -n "$CURRENT_HOSTNAME" ]]; then
    echo "      Current hostname appears to be: ${CURRENT_HOSTNAME}"
    echo "      Attempting to set hostname to: spinalfmriprep-dash"
    ...
    if tailscale up --hostname=spinalfmriprep-dash --reset 2>&1; then
        echo "[3/4] ✓ Hostname configured (using --reset)"
    else
        echo "[3/4] ⚠ Hostname configuration skipped (may need manual setup)"
        echo "      You can set it manually: tailscale up --hostname=spinalfmriprep-dash --accept-dns=false --accept-routes --advertise-tags=tag:server --operator=kiomars --ssh"
    fi
else
...
echo ""
echo "Service status:"
systemctl status spinalfmriprep-dashboard.service --no-pager -l | head -10 || true
echo ""
echo "Tailscale Serve status:"
tailscale serve status | grep -A 2 "17837" || echo "  (check with: tailscale serve status)"
echo ""
echo "Access the dashboard at:"
echo "  http://spinalfmriprep-dash:17837/"
echo ""
echo "Status endpoint:"
echo "  http://spinalfmriprep-dash:17837/__spinalfmriprep__/status.json"
echo ""
